...
| Other people open the workspace over Tailscale | Tailscale says who they are; a person whose email is their login gets in with that person's access (view reads, edit writes), the owner's devices as the owner, anyone else is told they asked and the owner is asked once; the chat, its questions, settings, imports and other workspaces are the owner's; access is set only by the owner's yes (let_in asks, taking it back does not), never through the API or a form; the log names who did it and on what | `internal/server/access_test.go`, `internal/chat/access_test.go`, `internal/tailnet/owner_test.go` |
