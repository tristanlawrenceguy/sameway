...
// TestAMarkCheckboxOnNoteListHasNoDuplicateLabel checks that on the /t/note
// listing page, each mark checkbox (the pin checkbox) has no duplicate label
// from an adjacent text node (Acceptance 1).
func TestAMarkCheckboxOnNoteListHasNoDuplicateLabel(t *testing.T) {
	_, h := newApp(t)

	// Create a note so it appears on the /t/note listing.
	...
	for _, input := range doc.Elements("input") {
		...
		found = true
		checkCheckboxRedundancy(t, input, "/t/note")
	}
	if !found {
		t.Fatal("expected at least one checkbox on the /t/note listing page")
	}
}

// TestAMarkCheckboxOnActionListHasNoDuplicateLabel checks that on the /t/action
// listing page, each mark checkbox has no duplicate label from an adjacent text node.
func TestAMarkCheckboxOnActionListHasNoDuplicateLabel(t *testing.T) {
	_, h := newApp(t)

	// Create an action so it appears on the /t/action listing.
	var act struct{ ID string }
	decode(t, postJSON(t, h, http.MethodPost, "/api/action",
		map[string]any{"title": "Water plants"}), &act)
	...
	for _, input := range doc.Elements("input") {
		...
		found = true
		checkCheckboxRedundancy(t, input, "/t/action")
	}
	if !found {
		t.Fatal("expected at least one checkbox on the /t/action listing page")
	}
}
