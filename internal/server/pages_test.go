...
// TestContentPagesLifecycle walks the human path: list with chat link, view via API-created record, delete.
func TestContentPagesLifecycle(t *testing.T) {
	a, h := newApp(t)

	// List page should link to /chat, not /new.
	list := parse(t, get(t, h, "/t/note"))
	if len(list.WithAttr("href", "/t/note/new")) > 0 {
		t.Fatalf("list page must not have a New note link")
	}
	if len(list.WithAttr("href", "/chat")) == 0 {
		t.Fatalf("list page needs a chat link")
	}

	// Create a record via the API (the human surface uses chat).
	...
	// Detail page must not have an edit link.
	for _, a := range detail.WithAttr("href", "") {
		href, _ := htmltest.Attr(a, "href")
		if strings.Contains(href, "/edit") {
			t.Errorf("detail page must not contain /edit: %s", href)
		}
	}
