...
// TestContentPagesLifecycle walks the human path: list, create via API,
// view detail, update via API, delete.
func TestContentPagesLifecycle(t *testing.T) {
	_, h := newApp(t)

	list := parse(t, get(t, h, "/t/note"))
	if len(list.WithAttr("href", "/t/note/new")) > 0 {
		t.Fatalf("list page should not have a New note link — form creation is removed")
	}

	// Create via API (the agent path).
	ok := postJSON(t, h, "POST", "/api/note", map[string]any{
		"title":  "Hello",
		"tags":   []string{"a", "b"},
		"pinned": true,
	})
	wantStatus(t, ok, http.StatusCreated)
	...
	// Update via API.
	upd := postJSON(t, h, "PUT", "/api/note/"+created.ID, map[string]any{
		"title":  "Hello again",
		"status": "published",
		"pinned": false,
	})
