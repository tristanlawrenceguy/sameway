...
// TestContentPagesLifecycle walks the human path: list, create via API, view, update via API, delete.
func TestContentPagesLifecycle(t *testing.T) {
	a, h := newApp(t)

	list := parse(t, get(t, h, "/t/note"))
	if len(list.WithAttr("href", "/t/note/new")) > 0 {
		t.Errorf("list page should not have a New note link")
	}

	// Create via API.
	rec, _ := a.Store.Create("note", map[string]any{"title": "Hello", "tags": []any{"a", "b"}, "pinned": true})
	...
	for _, l := range detail.WithAttr("href", detailPath+"/edit") {
		t.Errorf("detail should not have an edit link: %s", htmltest.Text(l))
	}

	// Update via API.
	rec, _ = a.Store.Update("note", rec.ID, map[string]any{"title": "Hello again"})
