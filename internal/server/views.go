...
func (s *Server) listPage(w http.ResponseWriter, r *http.Request) {
	...
	b.WriteString(string(s.component("link", map[string]any{"href": "/chat", "label": "Use chat to add"})))
	b.WriteString(`</div>`)
	if len(recs) == 0 {
...
// detailPage shows one record as a definition list with a delete link.
...
// display renders a stored value as read-only text for list and detail pages.
