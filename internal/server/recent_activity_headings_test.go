...
// activity section makes each entry's sentence an
...
func TestChatPageRecentActivityHasH3Headings(t *testing.T) {
	...
	// Acceptance 3: each entry's sentence is its own heading h3.
	if !strings.Contains(body, eventHeading) {
		t.Errorf("recent activity on /chat should make entries <h3 class=\"sw-event__text sw-event__heading\">\n%s", truncate(body))
	}

	// Acceptance 4: the h3 text is the full summary (e.g., "Assistant created note First note").
	...
	if h3Count != 2 {
		t.Errorf("expected 2 <h3 class=\"sw-event__text sw-event__heading\"> elements for 2 activities on /chat, got %d\n%s", h3Count, truncate(body))
	}
	...
	if slices.Contains(h3Said(body), "") {
		t.Error("no empty <h3> should appear on /chat")
	}
}
...
func TestChatPageRecentActivityEmptyStateHasNoHeading(t *testing.T) {
	...
	h3Count := strings.Count(body, eventHeading)
	if h3Count != 0 {
		t.Errorf("empty /chat should have 0 activity <h3> elements, got %d\n%s", h3Count, truncate(body))
	}
