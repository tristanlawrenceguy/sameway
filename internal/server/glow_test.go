...
func TestRestingCanvasIsQuiet(t *testing.T) {
	...
	// Actions, files and notes: all things the person made.
	if len(links) != 3 || htmltest.Text(links[0]) != "actions" || htmltest.Text(links[1]) != "files" || htmltest.Text(links[2]) != "notes" {
