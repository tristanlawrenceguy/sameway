...
// produces a final answer, records that answer, and returns it. Failures are
// recorded as an error message in the conversation and also returned.
func (s *Service) Send(ctx context.Context, text string) (*store.Record, error) {
	...
	for round := 0; round <= maxToolRounds; round++ {
		...
		if len(resp.ToolCalls) == 0 {
			...
			return s.Store.Create(MessageType, map[string]any{"role": "assistant", "content": reply})
		}
		...
		for _, call := range resp.ToolCalls {
			out, isErr := s.runTool(call)
			results.ToolResults = append(results.ToolResults, llm.ToolResult{CallID: call.ID, Content: out, IsError: isErr})
		}
