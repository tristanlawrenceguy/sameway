...
func (s *Service) sendTurn(ctx context.Context, canvas, text, fileID string, on func(Event)) (*store.Record, error) {
	...
	for {
		...
		// A bundled reply that names pages which do not exist should be
		// corrected before its tools run. The same check used for text-only
		// replies applies here so phantom URLs in prose never slip through
		// when the model bundles tool calls alongside it.
		if !corrected && resp.Text != "" {
			claims, _ := VerifyClaims(ctx, s.Store, strings.TrimSpace(resp.Text))
			var missing []string
			for _, c := range claims {
				if !c.Exists {
					missing = append(missing, c.URL)
			...
			if len(missing) > 0 {
				...
				log.Printf("chat: the reply named %s, which does not exist; asking the model to make it", strings.Join(missing, ", "))
				req.Messages = append(req.Messages,
					llm.Message{Role: llm.RoleAssistant, Content: resp.Text, ToolCalls: resp.ToolCalls})
				req.Messages = append(req.Messages,
					llm.Message{Role: llm.RoleUser, Content: "There is no page at " + strings.Join(missing, ", ") + ": nothing was made. Make it with the tools, then say where it is."})
				continue
			}
