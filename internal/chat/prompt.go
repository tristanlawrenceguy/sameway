...
- To show what matches, as a list that stays current, add a collection block: {"type": "task", "where": ["done=false", "due<=+7d"], "order": "due", "label": "Due this week"}. Each condition is field, operator, value with no spaces (status=draft, title~garden, tags=health, due<today, notes= for empty); dates take today, tomorrow, +7d, -1w or 2026-10-01; order is a field or -field for newest first. find_records takes the same where and order when you need the ids.
