...
// Replicate run.mjs exactly: use ONE page reused across all examples
...
let failures = 0;
for (const name of fs.readdirSync(path.join(root, 'design', 'components')).sort()) {
  const dir = path.join(root, 'design', 'components', name);
  ...
  for (const file of fs.readdirSync(examplesDir).filter((f) => f.endsWith('.html')).sort()) {
    ...
    // Before checking axe, dump computed styles for .sw-disclosure__body if it exists
    const bodyStyleBefore = await tab.evaluate(() => {
    ...
    if (name === 'disclosure') {
      console.log(`${name}/${file}: bodyStyle=${JSON.stringify(bodyStyleBefore)}, violations=${aa.violations.length}`);
