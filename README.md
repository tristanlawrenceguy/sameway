...
| Every record's page ends in one line: how many tasks are in this project, how many reminders are about it, what else is on its day. A count is a link that opens that list where you already are, and nothing else opens by itself | `internal/relate` works the connections out from the schema, in both directions: what points here, what is set about this page, what shares this record's parent, what falls on its day. `GET /api/<type>/<id>` and `get_record` return every one of them with the `where` that lists it, so the assistant knows what is there and can hand you `/t/<type>/<id>?show=<key>` when it has a reason |
